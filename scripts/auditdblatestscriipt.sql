...
-- Schema auditdb
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `auditdb` DEFAULT CHARACTER SET utf8mb4 ;
USE `auditdb` ;
USE `auditdb` ;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`getspecificcompliance`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_ActivateOrgHier`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_ActivateVendorForBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_ActivateVendorForCompany`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeactivateOrgHier`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeactivateVendorForBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeactivateVendorForCompany`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteComplianceBranchMapping`;
...
begin
DELETE FROM `auditdb`.`tbl_compliance_branch_mapping`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteRolePrivilege`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteRolePrivilege`(p_Role_ID int)
begin
DELETE FROM `auditdb`.`tbl_role_priv_map`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteUserGroupMembers`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteUserGroupMembers`(p_User_ID int)
begin
DELETE FROM `auditdb`.`tbl_user_group_members`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteUserRole`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteUserRole`(p_User_ID int)
begin
DELETE FROM `auditdb`.`tbl_user_role_map`
WHERE User_ID=p_User_ID;
end$$
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteUsergroup`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_DeleteUsergroup`(p_User_Group_ID int)
begin
DELETE FROM `auditdb`.`tbl_user_group`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_DeleteVendorForCompany`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteBranchAuditorMapping`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteBranchLocation`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteComplianceAudit`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteComplianceAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteComplianceOptionsXref`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteComplianceXrefAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteOrganizationHier`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteRole`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_deleteRole`(p_Role_ID int)
begin
UPDATE `auditdb`.`tbl_role`
SET
`Is_Active` = 0
WHERE `Role_ID` = p_Role_ID ;
end$$
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_deleteUser`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_deleteUser`(p_User_ID int)
begin
update `auditdb`.`tbl_user` set Is_Active=0 
where User_ID = p_User_ID;
end$$
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_fetchchangePassword`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getActs`;
...
then
SELECT * FROM `auditdb`.`tbl_compliance_xref`
where Comp_Category='Act' and `tbl_compliance_xref`.`level`=1;
else
SELECT *
FROM `auditdb`.`tbl_compliance_xref`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getAllCompanyBrnachAssignedtoAuditor`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getAllUser`;
...
SELECT `tbl_user`.`User_ID`,
    ...
    `tbl_user`.`Last_Login`,
    `tbl_user`.`Photo`
FROM `auditdb`.`tbl_user` where `tbl_user`.`Company_ID`=P_Company_ID and Is_Active=1;
end$$
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getAuditorforBranch`;
...
FROM `auditdb`.`tbl_branch_auditor_mapping`
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchAssociatedWithVendors`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchAuditorMapping`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchJoin`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchLocation`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getBranchesAssignedforAuditor`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCity`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCompaniesList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCompaniesListDropDown`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCompanieyList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCompanyLists`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCompanyListsforBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceAudit`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceBranchMapping`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceType`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceXref`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceXrefAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceXrefData`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getComplianceXreftype`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getCountry`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getDefaultCompanyLists`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getGroupCompaniesList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getGroupCompaniesListDropDown`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getGroupCompanyListActiveDeactive`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getGroupCompanyListDropDown`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getGroupHierJoin`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getLoginData`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getMenulist`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getMenus`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getOrganizationHier`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getOrganizationHierJoin`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getParticularGroupCompaniesList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getPrivilege`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getRole`;
...
FROM `auditdb`.`tbl_role` where Is_Active=1;
else
SELECT `tbl_role`.`Role_ID`,
    `tbl_role`.`Role_Name`,
    `tbl_role`.`Is_Active`,
    `tbl_role`.`Is_Group_Role`
FROM `auditdb`.`tbl_role`
WHERE `Role_ID` = p_Role_ID ;
end if;
end$$
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getRoleList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getRolePrivilege`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getRuleforBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getRules`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSections`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSpecificBranchList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSpecificBranchListDropDown`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSpecificVendorList`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSpecificVendorListDropDown`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getSpecifiySection`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getState`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getUser`;
...
SELECT `tbl_user`.`User_ID`,
    ...
    `tbl_user`.`Is_Active`,
    `tbl_user`.`Last_Login`
FROM `auditdb`.`tbl_user` where Is_Active=1;
else
SELECT `tbl_user`.`User_ID`,
    `tbl_user`.`User_Password`,
    `tbl_user`.`First_Name`,
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getUserGroup`;
...
FROM `auditdb`.`tbl_user_group`where Is_Active=1;
else
SELECT `tbl_user_group`.`User_Group_ID`,
    `tbl_user_group`.`User_Group_Name`,
    `tbl_user_group`.`User_Group_Description`,
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getUserGroupRole`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getUserRole`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getUserassignedGroup`;
...
FROM `auditdb`.`tbl_user_group` a left join tbl_user_group_members b on a.User_Group_ID=b.User_Group_ID
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getVendorJoin`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_getVendorsForBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertComplianceAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertComplianceXrefAuditTrail`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertLoginData`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertUserGroupMembers`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertUserGroupMembers`(p_User_ID int,p_User_Group_ID int)
begin
INSERT INTO `auditdb`.`tbl_user_group_members`
(`User_ID`,
`User_Group_ID`)
VALUES
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertUserRole`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertUserRole`(p_Role_ID int,p_User_ID int)
begin
INSERT INTO `auditdb`.`tbl_user_role_map`
(`Role_ID`,
`User_ID`)
VALUES
(p_Role_ID,p_User_ID);
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insert_User_Menu_Map`;

DELIMITER $$
USE `auditdb`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insert_User_Menu_Map`(p_UserGroup_Id int,p_Menu_Id int)
begin
INSERT INTO `auditdb`.`tbl_usergroup_menu_map`
(`User_Group_ID`,
`Menu_ID`)
VALUES
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateBranchAuditorMapping`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateBranchLocation`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateCompanyDetails`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateComplianceAudit`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateComplianceBranchMapping`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateComplianceXref`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateOrganizationHier`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateRole`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateUser`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateUserGroup`;
...
if(p_flag='I')
then
INSERT INTO `auditdb`.`tbl_user_group`
(`User_Group_Name`,
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_insertupdateVendorForBranch`;
...
USE `auditdb`;
DROP procedure IF EXISTS `auditdb`.`sp_updatePassword`;
