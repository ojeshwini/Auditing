...
SELECT `tbl_compliance_xref`.`Compliance_Xref_ID`,
    ...
    `tbl_compliance_xref`.`Audit_Type_ID`
FROM `auditdb`.`tbl_compliance_xref`
where Audit_Type_ID= p_Audit_Type_ID and Country_ID=p_Country_ID;
else
SELECT `tbl_compliance_xref`.`Compliance_Xref_ID`,
    `tbl_compliance_xref`.`Comp_Category`,
    `tbl_compliance_xref`.`Comp_Description`,
    ...
    `tbl_compliance_xref`.`Audit_Type_ID`
FROM `auditdb`.`tbl_compliance_xref`
where Audit_Type_ID= p_Audit_Type_ID and Country_ID=p_Country_ID and State_ID=p_State_ID and City_ID=p_City_ID;
end if;
end$$
...
p_Contact_Number,p_Gender,p_Is_Active,now(),p_Photo);
select last_insert_id();
end if;
else
UPDATE `tbl_user`
